    ...
    <div className="uploader-preview-card relative items-center w-full gap-4 overflow-hidden grid grid-cols-[1fr_36px]">
      <div className="z-[1] flex items-center w-full gap-6">
        {typeof file === 'undefined' && defaultValue && (
          ...
          >
            <img alt="" src={defaultValue} className="w-full h-auto fill" />
          </div>
        )}
    ...
    const { getRootProps, getInputProps } = useDropzone({
      ...
      onDrop: (acceptedFiles) => {
        ...
        if (duplicates.length > 0) {
          toast.warning('Some files already uploaded with same name');
        }
        ...
        const mergedFiles = multiple
          ? [...(files || []), ...uniqueNewFiles] // Merge without duplicates
          : uniqueNewFiles; // Replace if `multiple` is false
